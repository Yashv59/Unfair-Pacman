...
next_direction = DIRECTION_NONE  


window_width = 606
window_height = 606
window = pygame.display.set_mode((window_width, window_height))
pygame.display.set_caption("This Game is truly the Dark Souls Of Pacman")
...
for _ in range(num_enemies):
    ...
    while not valid_position:
        x = random.randint(30, window_width - 30)
        y = random.randint(30, window_height - 30)
        if not any(pygame.Rect(wall).colliderect(
            pygame.Rect(x - pacman_radius, y - pacman_radius, 2 * pacman_radius, 2 * pacman_radius)) for wall in walls):
            valid_position = True
                ...
                if len(enemies) < 50:
                    ...
                    while not valid_position:
                        ...
                        if not any(pygame.Rect(wall).colliderect(
                            pygame.Rect(x - pacman_radius, y - pacman_radius, 2 * pacman_radius, 2 * pacman_radius)) for wall in walls):
                            valid_position = True
                    enemies.append(Enemy(x, y, 10, 2 + len(enemies) * 0.1))
                    enemies.append(Enemy(x, y, 10, 2 + len(enemies) * 0.1))
